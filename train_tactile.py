...
class ModelTrainer(object):
    ...
    def load_dataset(self):
        train_dataset = TouchDataset(data_dir=self.data_dir,
                                     split='train',
        ...
        test_dataset = TouchDataset(data_dir=self.data_dir,
                                    split='test',
                                    balanced=True,
                                    n_frames=self.n_frames,
                                    do_augment=False)
    ...
    def get_criterion(beta=0.0):
        train_criterion = CPCE(beta=beta)
        test_criterion = SmoothedCrossEnropyLoss(add_softmax=True)
        return train_criterion, test_criterion

    def get_optimizer(self, lr, optim_name='adam', weight_decay=0.0, restrict_decay=False):
        if optim_name == 'adam':
        ...
        if restrict_decay is False or weight_decay == 0.0:
            optimizer = Optimizer(self.model.parameters(), lr=lr, weight_decay=weight_decay, **kwargs)
        else:
            decay_params = []
            no_decay_params = []
            ...
            param_groups = [{'params': decay_params, 'weight_decay': weight_decay},
                            {'params': no_decay_params, 'weight_decay': 0.0}]
            optimizer = Optimizer(param_groups, lr=lr, **kwargs)
    ...
    def train_model(self,
        ...
        self.optimizer = self.get_optimizer(lr=learning_rate, optim_name='sgd',
                                            weight_decay=weight_decay,
                                            restrict_decay=False)
...
if __name__ == '__main__':
    ...
    for N_FRAMES in range(1, 9):
        for run_number in range(1, 2):
            ...
            trainer = ModelTrainer(model, data_dir, chk_dir,
